...
type ColumnProps = {
  ...
  title: string;
  children: React.ReactNode;
  onAddTask: (task: Task) => void; // ✅ now required
};
...
export default function Column({ id, title, children, onAddTask }: ColumnProps) {
  ...
  const handleSubmit = () => {
    ...
    if (!onAddTask) {
      alert("❌ Add Task function not passed!");
      return;
    }
    ...
    onAddTask({
      ...
      id: Date.now().toString(),
      tag: task.tag || "Important",
      avatars: task.avatars?.length ? task.avatars : ["/avatars/avatar.png"],
      linkedin: task.linkedin || "",
      github: task.github || "",
      comments: typeof task.comments === "number" ? task.comments : 0,
    });
    ...
    alert("✅ Task added!");
    setTask({ id: "", title: "", description: "", date: "", tag: "Important", avatars: ["/avatars/avatar.png"], linkedin: "", github: "", comments: 0 });
    setAdding(false);
  };
  ...
  return (
    <div className="w-full sm:w-1/3 px-2">
      <div className="bg-gray-100 rounded-xl p-4 min-h-[300px]">
        ...
        {adding && (
          <div className="mb-4 bg-white p-4 rounded-lg shadow-sm text-sm ring-1 ring-gray-200 border-gray-200 space-y-2">
            <input
              placeholder="Project Name"
